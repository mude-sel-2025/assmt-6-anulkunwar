...
def confidence_interval(N_samples=40, sample_size=30, true_mean=806, true_std=11, confidence=0.95):
    """
    ...
    for i in range(N_samples):
        ...
        sample_se = sample.std()
        
        lower = sample_mean - z * sample_se/np.sqrt(N_samples)
        upper = sample_mean + z * sample_se/np.sqrt(N_samples)
        # this code block ends here
    ...
    for i, (low, up) in enumerate(zip(ci_lowers, ci_uppers)):
        ...
        color = 'red' if (true_mean < low or true_mean > up) else 'blue'
        # this code block ends here
